...
window.TOOTH_TIERS = [
  { from: 1, to: 3, icon: "🦷", baseName: "일반 치아", themeClass: "tooth-normal" },
  { from: 4, to: 6, icon: "🦴", baseName: "뿌리 치아", themeClass: "tooth-root" },
  { from: 7, to: 9, icon: "🛡️", baseName: "수호 치아", themeClass: "tooth-guard" },
  { from: 10, to: 12, icon: "⚜️", baseName: "성문장 치아", themeClass: "tooth-emblem" },
  { from: 13, to: 15, icon: "💎", baseName: "다이아 치아", themeClass: "tooth-diamond" },
  { from: 16, to: 18, icon: "🌋", baseName: "용암 치아", themeClass: "tooth-lava" },
  { from: 19, to: 21, icon: "🌌", baseName: "오로라 치아", themeClass: "tooth-aurora" },
  { from: 22, to: 24, icon: "👑", baseName: "왕관 치아", themeClass: "tooth-crown" },
  { from: 25, to: 25, icon: "✨👑✨", baseName: "초월 왕관 치아", themeClass: "tooth-transcend" }
];
...
function getToothName(level) {
  ...
  if (lv >= window.BALANCE.TRANSCEND_LEVEL) return "초월 왕관 치아";
  if (lv === window.BALANCE.MERGE_MAX_LEVEL) return "봉인된 왕관 치아";
...
function getToothDisplayLevel(level) {
  const lv = clampToothLevel(level);
  return lv >= window.BALANCE.TRANSCEND_LEVEL ? "Lv.MAX" : `Lv.${lv}`;
}
...
function canMergeLevel(level) {
  const lv = clampToothLevel(level);
  return lv >= 1 && lv < window.BALANCE.MERGE_MAX_LEVEL;
}
...
function getBaseAttackByLevel(level) {
  const lv = clampToothLevel(level);
  const table = {
    1: 10, 2: 18, 3: 32, 4: 58, 5: 105, 6: 190, 7: 340, 8: 610, 9: 1100,
    10: 1980, 11: 3560, 12: 6400, 13: 11500, 14: 20700, 15: 37300,
    16: 67100, 17: 120800, 18: 217400, 19: 391300, 20: 704300, 21: 1268000,
    22: 2282000, 23: 4108000, 24: 7394000, 25: 15000000
  };
  return table[lv] || Math.floor(10 * Math.pow(1.8, lv - 1));
}

window.getAtk = getBaseAttackByLevel;
...
window.toothIcons = Array.from({ length: 26 }, (_, index) => (index === 0 ? "" : getToothEmoji(index)));
window.toothNames = Array.from({ length: 26 }, (_, index) => (index === 0 ? "" : getToothName(index)));

window.PICKAXES = [
  { level: 1, icon: "🪵⛏️", name: "나무 곡괭이", power: 1, cost: 0, desc: "기본 채굴 도구" },
  { level: 2, icon: "🪨⛏️", name: "돌 곡괭이", power: 2, cost: 100, desc: "조금 더 단단한 곡괭이" },
  { level: 3, icon: "⚙️⛏️", name: "철 곡괭이", power: 5, cost: 1200, desc: "안정적인 채굴 성능" },
  { level: 4, icon: "🪙⛏️", name: "황금 곡괭이", power: 12, cost: 15000, desc: "황금빛 채굴 도구" },
  { level: 5, icon: "💎⛏️", name: "다이아 곡괭이", power: 30, cost: 240000, desc: "고급 광맥을 뚫는 곡괭이" },
  { level: 6, icon: "🔥⛏️", name: "용암 곡괭이", power: 75, cost: 4_500_000, desc: "뜨거운 광맥을 녹여 채굴" },
  { level: 7, icon: "⚡⛏️", name: "번개 곡괭이", power: 180, cost: 100_000_000, desc: "순간적으로 광석을 쪼개는 곡괭이" },
  { level: 8, icon: "🔱⛏️", name: "신화의 곡괭이", power: 420, cost: 3_000_000_000, desc: "최고 등급 채굴 도구" }
];
...
window.DUNGEON_THEMES = {
  forest: { name: "숲의 충치굴", className: "dungeon-theme-forest" },
  cave: { name: "회색 석회동굴", className: "dungeon-theme-cave" },
  ice: { name: "푸른 결정 폐허", className: "dungeon-theme-ice" },
  lava: { name: "용암 치근 지대", className: "dungeon-theme-lava" },
  royal: { name: "왕관의 성역", className: "dungeon-theme-royal" },
  boss: { name: "보스의 방", className: "dungeon-theme-boss" },
  hell: { name: "HELL 심연", className: "dungeon-theme-hell" },
  hellboss: { name: "HELL 보스 심장부", className: "dungeon-theme-hellboss" }
};

function getDungeonThemeKey(stage, mode) {
  const type = String(mode || "normal").toLowerCase();
...
window.STAGE_NAMES = [
  "입문자의 충치굴", "작은 법랑질 동굴", "초록 플라그 숲", "붉은 치석 협곡", "보라 세균 늪",
  "은빛 치근 광산", "황금 교합 평원", "다이아 법랑 성소", "용암 치수 지대", "오로라 치아 성역",
  "왕관의 문", "봉인된 왕관의 방", "심층 치근 통로", "균열의 법랑 성", "황금 교두 탑",
  "용암 신경관", "오로라 치주 숲", "왕관 파수꾼의 길", "봉인의 회랑", "왕관 성역 입구",
  "최후의 광맥", "잊힌 치아 신전", "초월 전야", "왕관의 끝", "치아 연대기"
];
...
window.TOOTH_DATA = {
  ...
  stageNames: window.STAGE_NAMES,
  dungeons: window.STAGE_NAMES,
  hellDungeons: window.STAGE_NAMES.map((name) => `HELL ${name}`),
  pickaxes: window.PICKAXES,
  dungeonMobs: [
  ...
  artifacts: [
    { id: "root_core", name: "치근의 핵", icon: "🦴", desc: "기본 공격력 증가", effectText: "전체 치아 공격력 증가" },
    { id: "enamel_shield", name: "법랑 방패", icon: "🛡️", desc: "용병 생존력 증가", effectText: "용병 체력 증가" },
    { id: "golden_cusp", name: "황금 교두", icon: "🪙", desc: "골드 획득량 증가", effectText: "골드 보상 증가" },
    { id: "diamond_pulp", name: "다이아 치수", icon: "💎", desc: "채굴 보상 강화", effectText: "채굴 보상 증가" },
    { id: "lava_root", name: "용암 치근", icon: "🌋", desc: "스플래시 공격 강화", effectText: "범위 피해 증가" },
    { id: "aurora_nerve", name: "오로라 신경", icon: "🌌", desc: "공격 속도 강화", effectText: "공격 쿨타임 감소" },
    { id: "crown_seal", name: "왕관의 봉인", icon: "👑", desc: "초월의 단서", effectText: "최종 각성의 흔적" },
    { id: "transcend_spark", name: "초월의 불꽃", icon: "✨", desc: "초월 왕관 치아의 흔적", effectText: "Lv.MAX의 증표" }
  ]
};

window.MERCENARIES = [
  { id: 0, name: "견습 치아병", icon: "🧑‍🚀", cost: 0, hp: 800, atkRate: 1, speed: 1, desc: "기본 용병" },
  { id: 1, name: "방패 치아병", icon: "🛡️", cost: 5000, hp: 1800, atkRate: 1.05, speed: 0.95, desc: "높은 체력" },
  { id: 2, name: "번개 궁수", icon: "⚡", cost: 80000, hp: 2400, atkRate: 1.2, speed: 1.15, desc: "빠른 공격" },
  { id: 3, name: "용암 기사", icon: "🌋", cost: 900000, hp: 5200, atkRate: 1.45, speed: 1, desc: "강한 공격력" },
  { id: 4, name: "오로라 사수", icon: "🌌", cost: 12000000, hp: 8500, atkRate: 1.8, speed: 1.25, desc: "상위 공격 용병" },
  { id: 5, name: "왕관 수호자", icon: "👑", cost: 250000000, hp: 18000, atkRate: 2.4, speed: 1.15, desc: "왕관급 최종 용병" }
];

window.TOOTH_DATA.mercenaries = window.MERCENARIES.map((m) => ({
  id: m.id,
  name: m.name,
  icon: m.icon,
  cost: m.cost,
...
window.TRAINING_TYPES = [
  { key: "hp", name: "체력 훈련", icon: "❤️", desc: "용병 최대 체력 증가", max: 50 },
  { key: "atk", name: "공격 훈련", icon: "⚔️", desc: "용병 공격력 증가", max: 50 },
  { key: "spd", name: "속도 훈련", icon: "🏃", desc: "이동 속도 증가", max: 30 },
  { key: "crit", name: "치명 훈련", icon: "💥", desc: "치명타 확률 증가", max: 30 },
  { key: "splashDmg", name: "범위 피해 훈련", icon: "🌊", desc: "스플래시 피해 증가", max: 30 },
  { key: "splashRange", name: "범위 확장 훈련", icon: "📡", desc: "스플래시 범위 증가", max: 30 }
];

window.REPLAY_VIDEOS = [
  { key: "intro", title: "인트로 영상", icon: "🎬", unlockType: "always", playFunction: "playIntroVideo" },
  { key: "hell", title: "지옥문 개방 영상", icon: "🔥", unlockType: "hell", playFunction: "playHellVideo" },
  { key: "awaken", title: "초월 왕관 치아 각성 영상", icon: "👑", unlockType: "awaken", playFunction: "playAwakenVideo" }
];
