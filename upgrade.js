// Version: 6.7.3 - Sticky Header & Diamond Fixes
...
window.renderShopItems = function() {
    ...
    // 모달 기본 패딩을 없애고 내부에서 조절하여 상단을 고정합니다.
    ...
    if (window.greatChanceLevel < MAX_GREAT_LV) {
        ...
        html += `<div class="shop-item"><div class="shop-info"><span>🍀 합성 대성공 확률업 (Lv.${window.greatChanceLevel})</span> <button onclick="buyItem('amulet', ${amuletCost})" class="btn-gold">💰 ${fNum(amuletCost)}</button></div><div class="shop-desc">치아 합성 시 2단계가 상승하는 대성공 확률을 증가시킵니다.<br><span style="color:#2ecc71;">대성공 확률: ${curGreat}% ➔ ${curGreat+2}%</span></div></div>`;
    } else {
    ...
    if (window.autoMineLevel < MAX_AUTO_MINE_LV) {
        ...
        html += `<div class="shop-item"><div class="shop-info"><span>🤖 자동채굴 속도업 (Lv.${window.autoMineLevel})</span> <button onclick="buyItem('auto', ${autoCost})" class="btn-gold">💰 ${fNum(autoCost)}</button></div><div class="shop-desc">방치형 오프라인 상태에서도 치아가 쌓이는 주기를 단축합니다.<br><span style="color:#2ecc71;">채굴 속도: ${curSpd}초 ➔ ${nextSpd}초</span></div></div>`;
    } else {
    ...
    if (expansionCount < 4) {
        ...
        html += `<div class="shop-item"><div class="shop-info"><span>🎒 인벤토리 확장 (${expansionCount+1}/4)</span> <button onclick="buyItem('exp', ${expCost})" class="btn-gold">💰 ${fNum(expCost)}</button></div><div class="shop-desc">치아를 보관할 수 있는 슬롯을 8칸 추가로 개방합니다. (최대 56칸)</div></div>`;
    } else {
...
window.renderRefineView = function() {
    ...
    <div style="grid-column: 1 / -1; background: #222; padding: 15px; border-radius: 8px; border: 2px solid var(--gold); margin-bottom: 5px;">
        ...
        <div style="display:flex; gap:10px;">
            ${isCdMax ? 
                ...
                `<button onclick="upgradeGlobal('cd', ${costGlobalCdGold}, ${costGlobalCdDia})" class="btn-sm" style="flex:1; height:70px; background:#34495e;">⏳ 쿨타임 Lv.${window.globalUpgrades.cd}<br><span style="color:#2ecc71;">-${curCdReduc}% ➔ -${nextCdReduc}%</span><br>💰${fNum(costGlobalCdGold)} ♦️${costGlobalCdDia}</button>`
            }
            ${isRngMax ?
                `<button class="btn-sm" style="flex:1; height:70px; background:#444; color:#888;">🏹 사거리 (MAX)<br>Lv.50</button>` :
                `<button onclick="upgradeGlobal('rng', ${costGlobalRngGold}, ${costGlobalRngDia})" class="btn-sm" style="flex:1; height:70px; background:#34495e;">🏹 사거리 Lv.${window.globalUpgrades.rng}<br><span style="color:#2ecc71;">Lv.${curRngVal} ➔ Lv.${curRngVal+1}</span><br>💰${fNum(costGlobalRngGold)} ♦️${costGlobalRngDia}</button>`
            }
        </div>
    </div>
    `;
    ...
    window.slotUpgrades.forEach((slot, idx) => {
        ...
        if (isLocked) {
            ...
            html += `<div class="refine-card"><div class="refine-header">🔥 슬롯 #${idx+1}</div><button class="refine-btn" onclick="upgradeSlot(${idx}, 'atk', ${costAtk})" style="height:100%;"><span>⚔️ 공격력 증폭 Lv.${slot.atk}</span><span class="refine-val" style="font-size:11px;">(x${curAtk} ➔ x${nextAtk})</span><span style="margin-top:5px; color:var(--gold);">💰 ${fNum(costAtk)}</span></button></div>`;
        }
    });
    grid.innerHTML = html;
};
...
window.renderTrainingList = function() {
    ...
    if(diaDisp) diaDisp.innerText = fNum(window.dia);
...
window.upgradeTraining = function(stat, costDia) {
    if (window.dia >= costDia) {
        window.dia -= costDia; window.trainingLevels[stat]++;
        try { playSfx('upgrade'); } catch(e){}
