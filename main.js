// Version: 6.9.3 - Final Restoration Engine (Monolithic Build)

// --- [ 1. 전역 변수 및 기초 데이터: 원장님 원본 구조 100% 보존 ] ---
window.gold = 0; 
window.dia = 0; 
...
const safeGetAtk = (lv) => typeof getAtk === 'function' ? getAtk(lv) : (10 * Math.pow(1.5, lv-1));
...
// 🌟 [버그 수정] 최고 기록 시간을 50158 -> 00:50 포맷으로 변환
...
function saveGame() {
    ...
    const data = { 
        ...
        bestClearTimes: window.bestClearTimes, lastTime: Date.now(), isMiningPaused: window.isMiningPaused 
    };
    ...
    try {
        ...
        if (d) {
            ...
            if (!window.isMiningPaused && d.lastTime) {
                ...
                for(let k=0; k < Math.floor((offTime * 1000) / currentMaxTime); k++) autoMergeLowest();
...
function gameLoop() { 
    if(!window.isMiningPaused) { 
        ...
        const currentMaxTime = Math.max(2000, 30000 - ((window.autoMergeSpeedLevel - 1) * 500)); 
        window.mergeProgress += (50 / currentMaxTime) * 100; 
...
window.setupMiningTouch = function() { 
    ...
    mineArea.addEventListener('pointerdown', (e) => { 
        ...
        let power = 15 + (window.pickaxeIdx * 10);
        ...
        if (window.highestToothLevel >= 4) power *= 1.2;
        processMining(power); 
        updateUI(); saveGame();
    }); 
};

// --- [ 6. 인벤토리 및 합성 엔진 (8칸 유지) ] ---
        ...
        if (i < window.maxSlots) { 
            slot.onpointerdown = (e) => { 
                if (window.inventory[i] > 0) { 
                    ...
                    if (curTime - lastTapTime < 300 && lastTapIdx === i) { 
                        e.preventDefault(); massMerge(window.inventory[i]); 
                        lastTapTime = 0; return; 
                    } 
                    lastTapTime = curTime; lastTapIdx = i; 
...
function autoMergeLowest() { 
    ...
    for(let i=8; i<window.maxSlots; i++) { 
        if (window.inventory[i] > 0) levelCounts[window.inventory[i]] = (levelCounts[window.inventory[i]] || 0) + 1; 
    } 
    let targetLv = Object.keys(levelCounts).map(Number).sort((a,b) => a - b).find(lv => levelCounts[lv] >= 2); 
    if (targetLv) massMerge(targetLv, true); 
}

function massMerge(lv, once = false) { 
    let indices = []; window.inventory.forEach((val, idx) => { if(idx >= 8 && val === lv && idx < window.maxSlots) indices.push(idx); }); 
    if(indices.length < 2) return; 
    try { if(typeof playSfx === 'function') playSfx('merge'); } catch(e){}
    const loopCount = once ? 1 : Math.floor(indices.length / 2); 
    for(let i=0; i < loopCount; i++) { 
        window.inventory[indices[2*i+1]] = Math.min(24, lv + 1); 
        window.inventory[indices[2*i]] = 0; 
        checkHighestTier(lv + 1); 
    } 
    if(getView() === 'mine') renderInventory(); 
}

// --- [ 7. 파트너 및 리얼 랭킹 시스템 (🌟 500명 통합) ] ---
window.renderPartnerCard = function() {
    ...
    card.innerHTML = `<div style="font-size:45px; width:60px;">${merc.icon}</div><div style="flex:1;"><div style="font-weight:bold; color:var(--gold); border-bottom:1px solid #555;">${merc.name} <span style="font-size:10px; color:#2ecc71;">고용중</span></div><div style="font-size:11px; color:#ddd; margin-top:4px;">⚔️ x${merc.atkMul} (+${atkB}%) | ❤️ ${safeFNum(merc.baseHp)} (+${hpB}%)</div></div>`;
};

window.generateRankings = function() {
    const list = document.getElementById('ranking-list');
    if(!list) return;
    let ranks = [{ name: "치아신", d: 40, p: 999999999 }, { name: "임플란트마스터", d: 35, p: 150000000 }, { name: "초보원장", d: 5, p: 15000 }];
    const prefix = ["User-", "Dr.", "Dent-", "Guest", "Tooth-"];
    const chars = "ABCDEF0123456789";
    for(let i=0; i<497; i++) {
        let lv = Math.floor(Math.random() * 40) + 1;
        let score = (Math.pow(1.6, lv) * 1000) + Math.floor(Math.random() * 50000);
        let rName = prefix[Math.floor(Math.random()*prefix.length)] + chars[Math.floor(Math.random()*chars.length)] + Math.floor(Math.random()*999);
        ranks.push({ name: rName, d: lv, p: score });
    }
    ...
    let myIdx = ranks.findIndex(r => r.isMe);
    let html = ''; 
    ranks.forEach((r, idx) => {
        if (idx < 30 || Math.abs(idx - myIdx) <= 2) {
            if (idx > 30 && idx === myIdx - 2) html += `<div style="text-align:center; color:#555; padding:5px;">⋮</div>`;
            ...
            html += `<div style="display:flex; justify-content:space-between; padding:8px 5px; border-bottom:1px solid #333; font-size:12px; ${style}">
                <span style="width:15%;">${idx+1}위</span><span style="flex:1; text-align:center;">${r.name}</span><span style="width:25%; text-align:right;">${safeFNum(r.p)}</span>
            </div>`;
        }
    });
    list.innerHTML = html;
    document.getElementById('my-rank-display').innerText = `내 순위: ${myIdx + 1}위 (전투력: ${safeFNum(myP)})`;
};

// --- [ 8. 던전 목록 및 기록 (🌟 시간 포맷 통합) ] ---
window.renderDungeonList = function() { 
    ...
    data.forEach((name, idx) => { 
        ...
        let rewardText = (idx + 1) % 2 === 0 ? `<p style="color:var(--gold); font-size:11px; margin:5px 0 0 0; font-weight:bold;">✨ 클리어 시 [Lv.${isHell ? 10 + Math.floor((idx+2)/2) : Math.floor((idx+2)/2) + 1} ${safeGetName(isHell ? 10 + Math.floor((idx+2)/2) : Math.floor((idx+2)/2) + 1)}] 확정!</p>` : "";

        if (open) { 
            div.innerHTML = `<h4>⚔️ Lv.${idx+1} ${name}</h4>${rewardText}<p style="margin:5px 0 0 0; font-size:11px; font-weight:bold; color:#00fbff;">🏆 최고 기록: ${formatClearTime(recTime)}</p>`;
            div.onclick = () => { if(typeof startDungeon === 'function') startDungeon(idx); };
        } else { div.innerHTML = `<h4>🔒 잠김</h4><p style="margin:5px 0 0 0; font-size:12px; color:#888;">이전 던전 클리어 시 해금</p>`; } 
...
window.equipMerc = function(id) { window.mercenaryIdx = id; window.renderMercenaryCamp(); window.renderPartnerCard(); saveGame(); };
